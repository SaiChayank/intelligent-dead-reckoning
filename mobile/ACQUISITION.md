...
../reports/android_acquisition_2026_09_19.md.

Use the build commands in README.md. For this workstation JAVA_HOME is

C:\Program Files\Android\Android Studio\jbr and the SDK is
...
Verified device observations — 2026-09-19

The procedure below was physically exercised on a OnePlus 12R / CPH2585 running Android 16 / API 36. The detailed evidence is in ../reports/android_acquisition_2026_09_19.md.

Observed real-device rates remained stable at approximately 98.79–98.80 Hz for accelerometer, 98.79–98.81 Hz for gyroscope, 49.39–49.40 Hz for gravity and 50.00 Hz for magnetometer. Precise-location testing produced GPS updates at approximately 0.997 Hz and exposed GNSS satellite status; approximate access remained network-provider only. NOT_REQUESTED, DENIED, APPROXIMATE, PRECISE and REVOKED permission states were all exercised.

Six-face gravity/accelerometer checks produced the expected dominant-axis sign reversals near 9.8 m/s², and deliberate rotations produced responses on gyroscope X/Y/Z. Backgrounding, recreation and source switching stopped acquisition without silent restart. dumpsys sensorservice and dumpsys location showed sensor/GPS/network registrations removed after Stop.

A >=30-minute stationary foreground run finished with 526,748 accepted records, 2 delayed, 0 duplicates, 0 dropped, 0 invalid, and a queue peak of 18/256. PSS samples were 167,518 kB, 173,084 kB, 157,229 kB and 155,967 kB; no monotonic memory growth was observed during the tested interval. These are observations from one device/session, not a general memory-leak proof or cross-device performance guarantee.

The following procedure is retained as the reproducible acceptance procedure for future devices/builds.

Apply the acquisition portions of foundation_readiness.md's device procedure:

Connect/unlock the phone, enable USB debugging and authorize this computer.

   Run adb devices -l; it must show the intended device as device. Set

   ANDROID_SERIAL if more than one device is listed.

Run gradlew.bat connectedDebugAndroidTest --offline --console=plain, then
...
Start simulation, switch to Phone sensors, verify simulation has stopped, then
...
While stationary, compare six phone faces for at least 10 seconds each and
...
Outdoors, inspect GPS fixes, measured rates, uncertainty and satellite counts.
...
Start/Stop, rotate/recreate the activity, press Home, return and switch source
...
Run a >=30-minute stationary foreground session. Inspect counters, bounded
...
SensorEvent timestamps and axes

Location elapsed time and optional fields

Precise/approximate runtime permissions

LocationManager subscriptions and GNSS status